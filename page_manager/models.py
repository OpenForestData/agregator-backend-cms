...
class MetaTagsExtension(PageExtension):
    ...
    title = models.CharField(max_length=500,
                             verbose_name="Tytuł (nadpisuje podstawowy tytuł)", null=True,
                             blank=True)
...
class MainPage(MetaPage, LangChooseMixin):
    ...
    youtube_link = models.CharField(max_length=500,
                                    verbose_name="Link do filmu na YouTube")
    youtube_mobie_text = models.CharField(max_length=120,
                                          verbose_name="Podpis pod filmem z YouTube")
    # mobile app section
    mobile_app_title = models.CharField(max_length=120,
                                        verbose_name="Tytuł sekcji aplikacja mobilna")
    mobile_app_content = models.TextField(
        verbose_name="Zawartość sekcji aplikacja mobilna", max_length=900)
    mobile_app_image = FilerImageField(verbose_name="Miniatura prezentująca aplikacje",
                                       on_delete=models.CASCADE,
                                       null=True, blank=True,
                                       related_name='mobile_app_image')
    mobile_app_cta_link = models.CharField(max_length=120,
                                           verbose_name="Link na przycisku")
    mobile_app_cta_text = models.CharField(max_length=120,
                                           verbose_name="Tekst na przycisku")
...
class IconSpecies(models.Model):
    ...
    """
    main_page = models.ForeignKey(MainPage, related_name="icon_species",
                                  on_delete=models.CASCADE)
    href = models.CharField(max_length=300, verbose_name="Link do przekierowania",
                            default="#")
    title = models.CharField(max_length=120, verbose_name="Nazwa Gatunku")
    image = FilerImageField(verbose_name="Miniatura prezentująca gatunek",
                            on_delete=models.CASCADE,
...
class FaqShort(LangChooseMixin):
    ...
    """
    title = models.CharField(max_length=120, verbose_name="Tytuł/Pytanie", null=True,
                             blank=True)
...
class AccordionPage(MetaPage):
    ...
    og_image = FilerImageField(verbose_name="Miniatura w social Media",
                               on_delete=models.CASCADE,
                               null=True, blank=True,
                               related_name='accordion_page_og_image')
...
class Accordion(models.Model):
    ...
    """
    accordion_page = models.ForeignKey(AccordionPage, related_name="accordion_page",
                                       on_delete=models.CASCADE)
...
class PagePattern(models.Model):
    ...
    title = models.ForeignKey(Title, unique=True, verbose_name="Strona",
                              ...
                              related_name="page_patterns")
    about_us = models.OneToOneField(AboutUsPage, unique=True,
                                    verbose_name="Szablon o Nas", related_name='about_us',
                                    on_delete=models.CASCADE, null=True, blank=True)
    accordion = models.OneToOneField(AccordionPage, unique=True,
                                     verbose_name="Szablon z Akordionami",
                                     related_name="accordion", on_delete=models.CASCADE,
                                     null=True, blank=True)
    ...
    def get_page_pattern_data(self) -> dict:
        ...
        if self.about_us_id is not None:
            try:
                og_image_thumb_url = get_thumbnailer(
                    self.about_us.og_image).get_thumbnail(
        ...
        if self.accordion_id is not None:
            ...
            try:
                og_image_thumb_url = get_thumbnailer(
                    accordion_page.og_image).get_thumbnail(options).url
            except Exception:
                og_image_thumb_url = ""
            page_pattern_data = {
                ...
                'accordions': [{'title': accordion.title, 'content': accordion.content}
                               for accordion in
                               Accordion.objects.filter(
                                   accordion_page=accordion_page).order_by('order')]
            }
